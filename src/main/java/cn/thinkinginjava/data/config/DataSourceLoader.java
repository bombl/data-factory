...
package cn.thinkinginjava.data.config;

import cn.thinkinginjava.data.exception.BussinessException;
 ...
 */
@Component
@ConditionalOnExpression("'${df.database.dialect}' == 'h2' || '${df.database.dialect}' == 'mysql'")
public class DataSourceLoader implements InstantiationAwareBeanPostProcessor {
    ...
    protected void init(final DataSourceProperties properties) {
        try {
            String jdbcUrl = StringUtils.replace(properties.getUrl(), "/df?", "?");
